...
dataPvaluesBetter <- read.table(file = "results/pvaluesBetter.txt", head = TRUE)
for (qi in QIs)
{
  ...
  png(file = paste0("results/plots/hasseDiagram", qi, ".png"))
  par(mar = c(0, 0, 0, 0))
  hasse(dataQI, ALGs, list(cluster = FALSE, transitiveReduction=TRUE))
  dev.off()
  
  png(file = paste0("results/plots/hasseDiagram", qi, "noTrans.png"))
  par(mar = c(0, 0, 0, 0))
